...
import logging
# import ssl # 더 이상 필요 없을 수 있음 (선택 사항)
...
if UPSTASH_REDIS_PASSWORD and UPSTASH_REDIS_ENDPOINT and UPSTASH_REDIS_PORT:
    ...
    FINAL_REDIS_URL = f"rediss://default:{UPSTASH_REDIS_PASSWORD}@{UPSTASH_REDIS_ENDPOINT}:{UPSTASH_REDIS_PORT}?ssl_cert_reqs=required" # Upstash 문서 가이드 적용
    logger.info("Using Upstash Redis for Celery with ssl_cert_reqs=required in URL.")
else:
...
# Redis SSL 설정 추가 - URL에서 처리하므로 주석 처리
# if FINAL_REDIS_URL.startswith("rediss://"):
#    celery_app.conf.broker_use_ssl = {'ssl_cert_reqs': ssl.CERT_NONE} # ssl.CERT_NONE 사용
#    celery_app.conf.redis_backend_use_ssl = {'ssl_cert_reqs': ssl.CERT_NONE} # ssl.CERT_NONE 사용
#    logger.info("Celery SSL/TLS enabled for Upstash Redis with ssl.CERT_NONE.")
