...
@celery_app.task(
    bind=True,
    name="celery_tasks.step_3_filter_content",
    max_retries=1,
    default_retry_delay=15,
)
def step_3_filter_content(
    self, prev_result: Dict[str, str], chain_log_id: str
) -> Dict[str, str]:
    ...
    logger.info(
        f"{log_prefix} ---------- Task started. Received prev_result_keys: {list(prev_result.keys()) if isinstance(prev_result, dict) else type(prev_result)} ----------"
    )
    ...
    if not isinstance(prev_result, dict) or "extracted_text" not in prev_result:
        ...
        _update_root_task_state(
            ...
            meta={
                "status_message": f"({step_log_id}) 오류: 이전 단계 결과 형식 오류 ('extracted_text' 누락)",
                "error": error_msg,
    ...
    if not extracted_text:
        error_msg = (
            f"Extracted text is missing from previous step result: {prev_result.keys()}"
        )
        logger.error(f"{log_prefix} {error_msg}")
        _update_root_task_state(
            root_task_id=chain_log_id,
            state=states.FAILURE,
            meta={
                "status_message": f"({step_log_id}) 이전 단계 텍스트 내용 없음",
                "error": error_msg,
                "current_task_id": task_id,
            },
        )
    ...
    if not raw_text_file_path or not isinstance(raw_text_file_path, str):
        logger.warning(
            f"{log_prefix} raw_text_file_path is invalid ({raw_text_file_path}). Will use placeholder for saving filtered file name."
        )
        base_text_fn_for_saving = (
            sanitize_filename(
                original_url if original_url != "N/A" else "unknown_source",
                ensure_unique=False,
            )
            + f"_{chain_log_id[:8]}"
        )
    else:
        base_text_fn_for_saving = os.path.splitext(
            os.path.basename(raw_text_file_path)
        )[0].replace("_extracted_text", "")

    logger.info(
        f"{log_prefix} Starting LLM filtering for text (length: {len(extracted_text)}). Associated raw_text_file_path for logging: {raw_text_file_path}"
    )
    self.update_state(
        state="PROGRESS",
        meta={
            "current_step": "채용공고 내용 필터링을 준비 중입니다.",
            "percentage": 0,
            "current_task_id": task_id,
    ...
    try:
        if not raw_text.strip():
            logger.warning(
                f"{log_prefix} Text file {raw_text_file_path} is empty. Saving as empty filtered file."
            )
            ...
            if not groq_api_key:
                ...
                _update_root_task_state(
                    ...
                    state=states.FAILURE,
                    meta={
                        "status_message": f"({step_log_id}) API 키 없음 (GROQ_API_KEY)",
            ...
            logger.debug(
                f"{log_prefix} GROQ_API_KEY: {'*' * (len(groq_api_key) - 4) + groq_api_key[-4:] if groq_api_key else 'Not Set'}"
            )

            chat = ChatGroq(
                temperature=0, groq_api_key=groq_api_key, model_name=llm_model
            )
            ...
            prompt = ChatPromptTemplate.from_messages(
                [("system", sys_prompt), ("human", human_template)]
            )
            ...
            logger.info(
                f"{log_prefix} Preparing to invoke LLM. Original text length: {len(raw_text)}"
            )
            MAX_LLM_INPUT_LEN = 24000
            text_for_llm = raw_text
            if len(raw_text) > MAX_LLM_INPUT_LEN:
                logger.warning(
                    f"{log_prefix} Text length ({len(raw_text)}) > limit ({MAX_LLM_INPUT_LEN}). Truncating."
                )
            ...
            logger.debug(
                f"{log_prefix} Text for LLM (first 500 chars): {text_for_llm[:500]}"
            )

            self.update_state(
                state="PROGRESS",
                meta={
                    "current_step": "LLM을 통해 채용공고 핵심 내용을 분석 중입니다...",
                    "percentage": 30,
                    "current_task_id": task_id,
            ...
            try:
                ...
                logger.info(
                    f"{log_prefix} <<< llm_chain.invoke completed. Duration: {duration_llm_invoke:.2f} seconds."
                )
                logger.info(
                    f"{log_prefix} LLM filtering complete. Output length: {len(filtered_content)}"
                )
                logger.debug(
                    f"{log_prefix} Filtered content (first 500 chars): {filtered_content[:500]}"
                )
                self.update_state(
                    state="PROGRESS",
                    meta={
                        "current_step": "채용공고 핵심 내용 분석 완료. 결과를 저장합니다.",
                        "percentage": 70,
                        "current_task_id": task_id,
                ...
                logger.error(
                    f"{log_prefix} !!! EXCEPTION during llm_chain.invoke: {type(e_llm_invoke).__name__} - {str(e_llm_invoke)}",
                    exc_info=True,
                )
                err_details_invoke = {
                    "error": str(e_llm_invoke),
                    "type": type(e_llm_invoke).__name__,
                    "traceback": traceback.format_exc(),
                    "context": "llm_chain.invoke",
                }
                self.update_state(
                    state=states.FAILURE,
                    meta={
                        "current_step": "오류: LLM 채용공고 분석 중 문제가 발생했습니다.",
                        "error": str(e_llm_invoke),
                        "type": type(e_llm_invoke).__name__,
                        "current_task_id": task_id,
                        "pipeline_step": "CONTENT_FILTERING_FAILED",
                ...
                _update_root_task_state(
                    ...
                    meta={
                        "status_message": f"({step_log_id}) LLM 호출 실패",
                        **err_details_invoke,
                        "current_task_id": task_id,
                        "pipeline_step": "CONTENT_FILTERING_FAILED",
                    },
        ...
        unique_filtered_fn = sanitize_filename(
            f"{base_text_fn_for_saving}_filtered_text", "txt", ensure_unique=True
        )
        filtered_text_file_path = os.path.join(logs_dir, unique_filtered_fn)

        logger.debug(
            f"{log_prefix} Writing filtered content (length: {len(filtered_content)}) to: {filtered_text_file_path}"
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={
                "current_step": "분석된 채용공고 내용을 안전하게 저장했습니다.",
                "percentage": 90,
                "current_task_id": task_id,
                "pipeline_step": "CONTENT_FILTERING_SAVED",
            },
        )
        _update_root_task_state(
            root_task_id=chain_log_id,
        ...
        result_to_return = {
            ...
            "cover_letter_preview": filtered_content[:500]
            + ("..." if len(filtered_content) > 500 else ""),
            "llm_model_used_for_cv": "N/A",
            "filtered_content": filtered_content,
        }
        logger.info(
            f"{log_prefix} ---------- Task finished successfully. Returning result. ----------"
        )
        logger.debug(
            f"{log_prefix} Returning from step_3: {result_to_return.keys()}, filtered_content length: {len(filtered_content)}"
        )
        self.update_state(
            state=states.SUCCESS,
            meta={
                **result_to_return,
                "current_step": "채용공고 내용 필터링이 성공적으로 완료되었습니다.",
                "percentage": 100,
                "pipeline_step": "CONTENT_FILTERING_SUCCESS",
            },
        )
        ...
        if filtered_text_file_path and os.path.exists(filtered_text_file_path):
            try:
                os.remove(filtered_text_file_path)
            except Exception as e_remove:
                logger.warning(
                    f"{log_prefix} Failed to remove partial filtered file {filtered_text_file_path}: {e_remove}"
                )

        err_details = {
            "error": str(e),
            "type": type(e).__name__,
            "filtered_file": raw_text_file_path,
            "traceback": traceback.format_exc(),
        }
        logger.error(
            f"{log_prefix} Attempting to update root task {chain_log_id} with pipeline FAILURE status due to exception. Error details: {err_details}"
        )
        self.update_state(
            state=states.FAILURE,
            meta={
                "current_step": "오류: 채용공고 내용 필터링 중 예기치 않은 문제가 발생했습니다.",
                **err_details,
                "current_task_id": task_id,
                "pipeline_step": "CONTENT_FILTERING_FAILED",
            },
        ...
        logger.error(
            f"{log_prefix} Root task {chain_log_id} updated with pipeline FAILURE status."
        )
        raise
