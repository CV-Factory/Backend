...
def generate_cover_letter(job_posting_content: str, user_story: str | None = None):
    ...
    query = f"""
    (RAG를 통해 이미 전달된) 채용 공고 내용과 다음 사용자 스토리를 기반으로 자기소개서를 작성해 주세요.
    자기소개서는 한국어로, 전문적이고 회사와 직무에 맞춰 작성되어야 합니다.
    사용자 스토리에서 관련 있는 기술과 경험을 강조하고, 이를 (RAG를 통해 이미 전달된) 채용 공고의 요구 사항과 연결해야 합니다.
    ...
    **매우 중요 지침: 출력은 반드시 제출 가능한 순수 자기소개서 본문 그 자체여야 합니다.
    ...
    단순히 기술을 나열하는 것이 아니라 사용자 스토리를 자기소개서에 자연스럽게 통합해야 합니다.
    만약 사용자 스토리가 제공되지 않았다면, (RAG를 통해 이미 전달된) 채용 공고 내용만을 기반으로 자기소개서를 생성하되, 사용자가 자신의 특정 경험을 추가해야 할 부분을 '여기에 경험을 추가하세요'처럼 명시적으로 언급하지 않고, 자기소개서 내용 안에서 미묘하게 암시하도록 작성해 주세요.

    사용자 스토리:
    {user_story if user_story else "제공되지 않음. (RAG를 통해 이미 전달된) 채용 공고를 바탕으로 자기소개서를 작성하고, 사용자의 경험에 따라 개인화할 부분을 미묘하게 암시해 주세요."}
